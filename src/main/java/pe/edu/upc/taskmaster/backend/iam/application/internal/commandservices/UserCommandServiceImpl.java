package pe.edu.upc.taskmaster.backend.iam.application.internal.commandservices;

import jakarta.transaction.Transactional;
import org.apache.commons.lang3.tuple.ImmutablePair;
import org.springframework.stereotype.Service;
import pe.edu.upc.taskmaster.backend.iam.application.internal.outboundservices.hashing.HashingService;
import pe.edu.upc.taskmaster.backend.iam.application.internal.outboundservices.tokens.TokenService;
import pe.edu.upc.taskmaster.backend.iam.domain.model.aggregates.User;
import pe.edu.upc.taskmaster.backend.iam.domain.model.commands.*;
import pe.edu.upc.taskmaster.backend.iam.domain.services.UserCommandService;
import pe.edu.upc.taskmaster.backend.iam.infrastructure.persistence.jpa.repositories.RoleRepository;
...
public class UserCommandServiceImpl implements UserCommandService {
    ...
    */
    ...
    public void handle(ChangePasswordCommand changePasswordCommand) {
        var userOptional = userRepository.findById(changePasswordCommand.userId());
        if (userOptional.isEmpty()) {
        ...
        if (!hashingService.matches(changePasswordCommand.currentPassword(), user.getPassword())) {
            throw new IllegalArgumentException("Current password is incorrect");
        }

        user.setPassword(hashingService.encode(changePasswordCommand.newPassword()));
        userRepository.save(user);
    }
}
